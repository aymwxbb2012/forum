...
public class ChannelDao extends BaseDao<Channel> implements IChannelDao {
	...
	public List<Channel> listQTopNavChannel() {
		String hql = "select new Channel(c.id,c.name,c.customLink,c.customLinkUrl) " +
				"from Channel c where c.status=0 and c.isTopNav=1 and c.type in ("+ChannelType.QUESTION_LIST.ordinal()+","+ ChannelType.QUESTION_CONTENT.ordinal()+") order by navOrder";
		return this.list(hql);
	}
	...
	public List<Channel> listTTopNavChannel() {
		String hql = "select new Channel(c.id,c.name,c.customLink,c.customLinkUrl) " +
				"from Channel c where c.status=0 and c.isTopNav=1 and c.type in ("+ChannelType.TOPIC_LIST.ordinal()+","+ ChannelType.TOPIC_CONTENT.ordinal()+")   order by navOrder";
		return this.list(hql);
	}
