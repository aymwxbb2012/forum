(function($){
	...
	$.fn.mysorttable = function(opts) {
		...
		function beginOrders() {
			if(!_isSort) {
				$(_that).find("thead tr").append("<td>sequence number</td>");
				setOrders();
				$(_that).find("tfoot tr").append("<td>Drag to sort</td>");
				sortEle.sortable("enable");
				_isSort = true;
			} else {
				alert("Already in the sorted state");
			}
		}
	...
	$.fn.confirmOperator = function(opts) {
		var settings = $.extend({
			msg:"The operation is irreversible.Are you sure?",
			eventName:"click"
		},opts||{});
